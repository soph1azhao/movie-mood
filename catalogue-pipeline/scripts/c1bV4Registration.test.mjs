...
describe('C1b-V4 static registration', () => {
  ...
  it('binds the registered protocol to closure, triage, cohort, contracts, and same-lock JIT order', async () => {
    ...
    expect(protocol.historicalProvenance.predecessorProtocol).toBe('phase-5c-c1b-v-confirmatory.v3')
    expect(protocol.stages[1].sourceProtocolId).toBe('phase-5c-c1b-v-confirmatory.v3')
    expect(bundle.contracts.find(({ id }) => id === 'imported-v3-cohort-contract.v4').canonicalContent.sourceProtocolId).toBe('phase-5c-c1b-v-confirmatory.v3')
    expect(triage.importedCohort.sourceProtocolId).toBe('phase-5c-c1b-v-confirmatory.v3')
    expect(protocol.studyEstimands.coverage.coverageByFriendsGold).toBe('NOT_IDENTIFIABLE_IN_MAIN_V4_DESIGN')
    expect(protocol.stages.find(({ stage }) => stage === 3).lowConfidenceArchive.path).toBe('c1b-v4-boundary-pool.json')
    expect(JSON.stringify(protocol)).not.toContain('automaticallyExecutedByV3')
    expect(protocol.requiredDesignDisclosures).not.toHaveProperty('annualPartitionSelectionBasis')
    expect(protocol.requiredDesignDisclosures).not.toHaveProperty('annualCellPageCap')
    expect(protocol.forbiddenAdaptationsAfterExecutionBegins.join(' ')).not.toMatch(/annual|page-plan|V2 raw page/u)
  ...
  it('executes identity through viability on the declared synthetic fixture with zero external requests', async () => {
    ...
    expect(provenance.fixtures[0].selectionRationale).toContain('without using any V4 candidate-specific Wikipedia outcome')
