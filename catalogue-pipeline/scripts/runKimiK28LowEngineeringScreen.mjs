...
export async function runLowScreen({ limit, pipelineRoot = resolve('catalogue-pipeline'), env = process.env, fetchImpl = globalThis.fetch, now = () => Date.now(), readJsonFile = readJson, readTextFile = readFile, writeJsonFile = writeJson, fileExists = exists, buildPreflight = buildLowScreenPreflight, classifyFn = classifySemanticCandidate, acquireLock = acquireRunLock, releaseLock = releaseRunLock } = {}) {
  ...
  const preflight = await buildPreflight({ pipelineRoot, readJsonFile, readTextFile, fileExists }); const lock = await acquireLock(preflight.runtime.lockPath, { runId: RUN_ID, acquiredAt: new Date().toISOString() })
  try {
